...
class User
{
    ...
    public function login($usernameOrEmail, $password, $rememberme)
    {
        ...
        if (empty($usernameOrEmail) || empty($password)) {
            return ['Make sure you fill all boxed in!'];
        }

        if (true === $this->isValidEmail($usernameOrEmail)) {
        ...
        if (is_array($user)) {
            ...
            if (password_verify($password, $user['password'])) {
                if ($this->bruteCheck($user['id'])) {
                    return ['Your account has been locked out', 'Please try to login again later!'];
                }
    ...
    public function register($username, $password, $confirmpassword, $email, $agree = true)
    {
        ...
        $database = Database::getInstance();
        if (false === $database->query($sql, $sqlOptions)) {
            return ['Unable to create account, contact site admin!'];
        }

        return ['Created account, you can now login!'];
    }
